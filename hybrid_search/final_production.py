...
def hybrid_retrieve(query, retrievers, weights, k = 3, rrf_k = 60):
    ...
    # 2. Sort the dictionary by the accumulated scores (highest to lowest)
    sorted_docs = sorted(doc_scores.values(), key=lambda x: x[0], reverse=True)
    
    # 3. Extract just the Document objects (dropping the raw scores) and return top k
    return [doc_tuple[1] for doc_tuple in sorted_docs[:k]]
...
class HybridRetriever:
    """Production hybrd retriever with BM25 + Vector Search"""
    ...
    def add_documents(self, documents: List[Document]):
        ...
        # Add to vector store
        self.vectore.add_documents(documents)
